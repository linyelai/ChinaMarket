...
<template>
  <div id="register">
    <el-dialog title="Sign up" width="300px" center :visible.sync="isRegister">
      <el-form
        :model="RegisterUser"
        :rules="rules"
        status-icon
      ...
      >
        ...
        <el-form-item>
          <el-button size="medium" type="primary" @click="Register" style="width:100%;">Sign up</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
  ...
  data() {
    ...
    let validatePass = (rule, value, callback) => {
      ...
      if (passwordRule.test(value)) {
        ...
        return callback(
          new Error("Start with a letter, length 6-18, allow alphanumeric underscore")
        );
      }
    };
    // 确认密码的校验方法
    let validateConfirmPass = (rule, value, callback) => {
      ...
      if (this.RegisterUser.pass != "" && value === this.RegisterUser.pass) {
        ...
        return callback(new Error("The two passwords entered are inconsistent "));
      }
    };
    return {
